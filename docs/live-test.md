...
The current development build is `0.1.1`.
...
3. Codex uses synchronous `request_user_input` rather than `request_user_input_async`.
4. The root session waits for the answer; dependent implementation does not continue in the background.
5. After the answer, the decision becomes `resolved`, the answer is stored, and only satisfied tasks are unblocked.
...
## Test 4 — fallback when synchronous input is unavailable

If the client cannot use synchronous `request_user_input`, Codex must:
...
- the root converts the question into a blocking decision and asks the user;
